...
public class ConsoleUtils {
	...
	/*
	 * 219 DB U+2588 █ full block 220 DC U+2584 ▄ lower half block 221 DD U+258C
	 * ▌ left half block 222 DE U+2590 ▐ right half block 223 DF U+2580 ▀ upper
	 * half block
	 */
	...
	public static String getDeviceDesc(Capabilities caps) {
		if (ConfigurationUtils.isDevice(caps))
			...
			return getPlatformName(caps) + " " + getDeviceName(caps) + " " + getVersion(caps);
	}

	public static String getDeviceName(Capabilities caps) {
		return caps.getCapability("deviceName") == null ? (caps.getCapability("deviceDbName") == null
				? (caps.getCapability("description") == null ? "" : caps.getCapability("description") + "")
				: caps.getCapability("deviceDbName") + "") : caps.getCapability("deviceName") + "";
	}
	...
	public static String getPlatformName(Capabilities caps) {
		return caps.getCapability("platformName") == null
				? (ConfigurationUtils.isDesktopBrowser(caps)
						? ("ANY".equals(caps.getCapability("platform")) ? "Desktop"
								: caps.getCapability("platform") + "")
	...
	public static String getVersion(Capabilities caps) {
		return caps.getCapability("platformVersion") == null
				? (caps.getVersion() == null || caps.getVersion().isEmpty() ? caps.getBrowserName() : caps.getVersion())
